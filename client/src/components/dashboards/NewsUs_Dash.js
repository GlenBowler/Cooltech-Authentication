...
import './DashUs.css';
//Coponent that will show us the News user Dashboard
class NewsUsDash extends React.Component {
constructor (props){
    super(props);
//Setting our states
this.state = {
...
componentDidMount () {
    //Fetching from routes. Geting all the users for user in the News reviews unit
    fetch("/api/findCred?ou=News Reviews")
    .then(res => res.json())
    .then(
        (result) => {
...
render () {
    ...
    //1: See all the user data in form of table
    else {
        return <div>
            <link rel="stylesheet" href="/stylesheets/style.css"/>
            <h1 className="title">Cool Tech</h1>
            <h2 className="titleDos">News Reviews user Side</h2>
