...
exports.updateRole=function (req,resp){
    ...
    //Update user role
    Schema.findOneAndUpdate({username: req.query.username}, {role: req.query.role}, {new: true}, function (err){
        if(err) {
            console.log("There was some sort of issue while trying to update the user role");
        }
            resp.send("updating user role was succesfull");
        });
}
